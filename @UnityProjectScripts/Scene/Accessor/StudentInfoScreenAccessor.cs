// AUTO GENERATED SOURCE. DO NOT MODIFY.

using UnityEngine;
using UnityEngine.UI;

public class StudentInfoScreenAccessor : MonoBehaviour
{
    public Image ExpBar;
    public Text LevelText;
    public Image AttributeIcon;
    public Text AttributeText;
    public Text NameText;
    public GameObject StarHolder;
    public Image TabButtonBox_BasicTabButton;
    public Text TabButtonBox_BasicTabButton_Text;
    public Image TabButtonBox_LevelUpTabButton;
    public Text TabButtonBox_LevelUpTabButton_Text;
    public Image TabButtonBox_ShinbiTabButton;
    public Text TabButtonBox_ShinbiTabButton_Text;
    public Text BasicTab_ExSkillInfo_SkillDescriptionText;
    public Text BasicTab_ExSkillInfo_SkillNameText;
    public Image BasicTab_ExSkillInfo_Badge;
    public Image BasicTab_ExSkillInfo_Thumbnail;
    public Image BasicTab_ExSkillInfo_LevelImage;
    public Image BasicTab_StatInfo_SDFullShot;
    public Text BasicTab_StatInfo_StatPage1Value;
    public Text BasicTab_StatInfo_StatPage2Value;
    public Text BasicTab_WeaponInfo_WeaponTypeText;
    public GameObject BasicTab_WeaponInfo_StarHolder;
    public Image BasicTab_WeaponInfo_WeaponImage;
    public GameObject FullshotParent;
    public Image FullshotImage;
    public Image FullshotHaloImage;
    public Image BackButton;
}
